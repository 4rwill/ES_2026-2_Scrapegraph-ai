...
## Conclusão
As verificações automáticas existem mas não são exatamente otimizadas. Existem muitos problemas que podem passar despercebidos por conta das verificações estarem 'frouxas'. O check de lint não provoca bloqueio e o check de testes não valida todos os testes, apenas os que não são de integração.
